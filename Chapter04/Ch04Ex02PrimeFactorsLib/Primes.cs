namespace Ch04Ex02PrimeFactorsLib
{
    public class Primes
    {
        ...
        {
            // If user number is a prime number than factor is the number
            if (PrimeNumbers.Contains(inputNumber))
            {
                return $"Prime factor of {inputNumber} is {inputNumber}";
            }
            // If user number is not prime, we will need to find factors with division
            ...
            foreach (int pn in PrimeNumbers)
            {
                ...
                do
                {
                    remainder = inputNumber % pn;
                    if (remainder == 0)
                    {
                        inputNumber = inputNumber / pn;
                        if (inputNumber == 1)
                        {
                            factors += $"{pn}";
                        }
